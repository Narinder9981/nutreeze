...
class WSSuspendplanwithmultipledatesController extends WSBaseController {

    /**
     * @Route("/ws/suspendplanwithmultipledates/{param}",defaults = {"param"=""},requirements={"param"=".+"})
     */
    public function suspendplanwithmultipledatesAction($param) {

        try {
            $this->title = "Freeze Subpackage with Multiple Dates";
            $this->status = 200;
            $this->message = true;
            ...
            if ($this->validateData($param)) {
                ...
                $freeze_dates = json_decode($param->freeze_dates, true);
                ...
                if($freeze_dates){
                    foreach($freeze_dates as $frkey=>$frval){
                        if (strtotime($frval) < strtotime($time_after_two_days)) {
                            $this->error = "PIW";
                            $this->status = 201;
                            $this->message = false;
                            $this->error_msg = "Cant Suspend before ".$this->SELECT_MEAL_AFTER_DAYS."  Day. Your Freeze date : " . $frval . " is not applicable " ;
                            return $this->responseAction();
                        }
                    }
                }
                ...
                //-----------start from here -update code ----------------

                foreach($freeze_dates as $freeze_date_key=>$freeze_dateval){
                    $check_already_freeze_Query = "SELECT * FROM `freeze_subpackage` WHERE `order_id` = ".$order_id." AND `is_deleted` = 0 and sub_package_id = ".$sub_package_id." and ( start_date = '".$freeze_dateval."' or end_date = '".$freeze_dateval."' or (start_date>= '".$freeze_dateval."' and end_date <= '".$freeze_dateval."'))" ; 
                    $check_already_freeze = $this->fireQuery($check_already_freeze_Query) ; 
                    if(empty($check_already_freeze)){
                        $start_date = strtotime($freeze_dateval);
                        $end_date = strtotime($freeze_dateval);
