...
class WSUpgradePackageBySubPackageController extends WSBaseController {

    /**
     * @Route("/ws/upgradePackageBySubPackage/{param}",defaults = {"param"=""},requirements={"param"=".+"})
     * @Template()
     */
    public function upgradePackageBySubPackageAction($param) {
        try {
            $this->title = "Upgrade Package By Subpackage";
            $this->status = 200;
        	$this->message = true;
            ...
            $this->validateRule = array(
                array(
                    'rule' => 'NOTNULL',
                    'field' => array('new_sub_package_id', 'user_id', 'order_id', 
                    'payment_type', 'total_amount','current_sub_package_id'),
                ),
            );
            if ($this->validateData($param)) {

                $new_sub_package_id = $param->new_sub_package_id;
                $current_sub_package_id = $param->current_sub_package_id ; 
                ...
                if ($orderMaster) {
                    ...
                    if (empty($checkMonthlyPackage)) {
                        $this->error = "PIW";
                        $this->error_msg = "Sub package is not Monthly package , you can upgrade in monthly package only ";
                        // $this->data = false;
                        $this->status = 201;
                        $this->message = false;
                        $emptyObj = new Countrymaster();
