...
class WSPackagelistForupgradeController extends WSBaseController {

    /**
     * @Route("/ws/packagelistforupgrade/{param}",defaults = {"param"=""},requirements={"param"=".+"})
     * @Template()
     */
    public function packagelistforupgradeAction($param) {

        try {
            $this->title = "Package List for upgrade";
            $this->status = 200;
            $this->message = true;
            ...
            $this->validateRule = array(
                array(
                    'rule' => 'NOTNULL',
                    'field' => array('lang_id', 'package_id','order_id'),
                ),
                    );
            ...
            if ($this->validateData($param)) {
                ...
                $order_info = $this->getDoctrine()->getManager()->getRepository('AdminBundle:Ordermaster')->findOneBy(array("order_master_id"=>$order_id));
                if(empty($order_info)){
                    $this->error = "PIW";
                ...
                if($package_id != $order_info->getPackage_id()){
                    $this->error = "PIW";
                    $this->error_msg = "Order Package and PAckage id are wrong ";
                    $this->status = 201;
                    $this->message = false;
                    $emptyObj = new Subpackagemaster();
                    $this->data = $emptyObj;
                ...
                $sub_package_id = $order_info->getSub_package_id() ;
                $package_for_order_id = $order_info->getPackage_for() ;
                $sub_package_combination_master = $this->getDoctrine()->getManager()->getRepository("AdminBundle:Subpackagecombinationmaster")
                    ->findBy(array("sub_package_id"=>$sub_package_id,"is_deleted"=>0));
              
                $sub_package_meal_types = [] ;
                if(!empty($sub_package_combination_master)){
                    foreach($sub_package_combination_master as $subpkey=>$subpval){
                        $sub_package_meal_types[$subpval->getMeal_type_id()] = $subpval->getMeal_value() ; 
                    }
                }
                ...
                if($packageMaster){
                    ...
                    if(!empty($pk_for)){
                        foreach($pk_for as $_pk_for){
                            ...
                            and sub_package_price_master.duration_type = '$duration_db_name' and sub_package_master.main_package_id = '$main_package_id' and sub_package_master.language_id = '$lang_id' and sub_package_master.main_sub_package_id != " . $sub_package_id;
                            ...
                            if(!empty($sub_packages)){
								...
								//	echo "<br>Price : " . $price . " price_per_day " . $price_per_day  . " ID " . $sub_pk_id ;
                                    if($price > 0 && $price_per_day > 0 ){
                                        ...
                                        $Dispaly_in_response = false ;                                        
                                        if (
                                            //( ($sub_package_meal_types[1] + $sub_package_meal_types[2]) <= $totalMeals ) && 
                                             /*($sub_package_meal_types[2]  <= $totalMeals ) && 
                                            ( $sub_package_meal_types[1]  <= $totalBreakfast ) && 
                                            ( $sub_package_meal_types[3]  <= $totalSnacks ) && 
                                            ( $sub_package_meal_types[11]  <= $totalSoup )
                                            ) */  

											($sub_package_meal_types[2]  <= $totalMeals ) && 
                                            ( $sub_package_meal_types[1]  <= $totalBreakfast ) && 
                                            ( $sub_package_meal_types[3] + $sub_package_meal_types[11]  <= $totalSnacks ) 
                                            
                                            )   
                                        {
                                            $Dispaly_in_response = true ; 
                                        }
										 
                                        if($Dispaly_in_response == true){
                                            if($display_package_as_it_is == false){
                                                ...
                                                $sub_packages_data [] = array(
                                                    ...
                                                    "max_limit" => $_sub_packages['max_limit'],
                                                    "price" => $price,
                                                    "price_per_day"=>($price / $_pk_for['days'] ),
                                                    "actual_package_price" => $price,
                                                    "total_combo_count" => $totalComboCount,
                                                    "sub_package_combo" => $sub_packages_combo,
                                                ); 
