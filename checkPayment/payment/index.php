...
$expression = $custname &&   $custemail && $custemobile  && $totAmount  && $productnames && $membership_id && $user_id && $insid ;
        ...
        if ($ResponseCode == 0) {
			if($referenceID){
				...
				$current_date = date("Y-m-d H:i:s");
                        ...
                        $select = "SELECT * FROM `order_master` WHERE order_id_app='$udf1'";
                        $res = db_query($select);
                        $count = mysqli_num_rows($res);
                        if($count == 0){
                        $insert = "INSERT INTO `order_master` (`transaction_id`, `order_date`, `user_id`, `membership_id`, `customer_name`, `customer_email`, `customer_phone`, `total_amount`, `order_status`, `order_id_app`, `is_deleted`) VALUES (0,'$current_date','$user_id','$membership_id','$custname','$custemail','$custemobile','$totAmount','pending','$udf1',1)";
